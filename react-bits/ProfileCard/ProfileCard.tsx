...
const DEFAULT_BEHIND_GRADIENT = `
  radial-gradient(farthest-side circle at var(--pointer-x) var(--pointer-y),
    hsla(260, 100%, 90%, var(--card-opacity)) 4%,
    hsla(260, 50%, 80%, calc(var(--card-opacity)*0.75)) 10%,
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  return (
    <div ref={wrapRef} className={`pc-card-wrapper ${className}`.trim()} style={cardStyle}>
      <section ref={cardRef} className="pc-card starry-bg">
        <div className="pc-inside">
          ...
          <div className="pc-content">
            <div className="pc-details">
              <h3 style={{ color: 'hsl(241, 85%, 68%)' }}>{name}</h3>
              <br />
              <p>{title}</p>
            </div>
          </div>
